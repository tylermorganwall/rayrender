...
test_that("render_scene basic options", {
  ...
  scene1 = generate_ground(
    ...
    add_object(sphere(
      ...
      z = 0,
      radius = 0.5,
      material = diffuse(color = c(1, 0, 1))
    )) |>
    ...
    add_object(cylinder(
      ...
      z = 0,
      radius = 0.5,
      material = metal(color = "gold", fuzz = 0.02)
    )) |>
    ...
    add_object(cone(
      ...
      material = glossy(
        color = "green",
        gradient_point_start = c(0, 0.7, 0),
        gradient_point_end = c(0, 2.1, 0),
        gradient_color = "red"
      )
    )) |>
  ...
  render_args_camera_info = expand.grid(
    lookfrom = list(c(0, 1, 10), c(-10, 1, 10)),
  ...
  render_args_post_processing_info = expand.grid(
    bloom = list(FALSE, 10),
    clamp_value = list(Inf, 10),
    tonemap = list("raw", "reinhard", "uncharted", "hbd")
  )

  run_tests(
    "render_scene",
    render_args_post_processing_info,
    plot_prefix = "post",
    list(
      scene = scene1,
      lookat = c(0, 1, 0),
  ...
  render_sampling_args = expand.grid(
    samples = list(1, 16),
  ...
  run_tests(
    "render_scene",
    render_sampling_args,
    plot_prefix = "sampling",
    list(
      scene = scene1,
      lookat = c(0, 1, 0),
      plot_scene = FALSE,
  ...
  render_termination_args = expand.grid(
    max_depth = list(NA, 10, 100),
    roulette_active_depth = list(100, 10, 3)
  )

  run_tests(
    "render_scene",
    render_termination_args,
    plot_prefix = "termination",
    list(
      scene = scene1,
      lookat = c(0, 1, 0),
      plot_scene = FALSE,
  ...
  render_realistic_camera_args = expand.grid(
    camera_description_file = list("telephoto", "50mm", "wide", "fisheye"),
    aperture = list(1, 10),
    camera_scale = list(1, 3),
    film_size = list(22, 40)
  )
  #Only include a few examples of 40mm film
  render_realistic_camera_args = render_realistic_camera_args[
    c(1, 4:8, 16, 25, 30),
  ]

  run_tests(
    "render_scene",
    render_realistic_camera_args,
    plot_prefix = "real_cam",
    list(
      scene = scene1,
      lookat = c(0, 1, 0),
      plot_scene = FALSE,
...
test_that("render_scene accepts rayvertex ray_mesh scenes", {
  ...
  expect_false(
    raymesh_model(scene)$shape_info[[1]]$shape_properties$override_material
  )
  expect_true(
    raymesh_model(scene, material = diffuse())$shape_info[[
      1
    ]]$shape_properties$override_material
  )
  expect_true(
    raymesh_model(scene, override_material = TRUE)$shape_info[[
      1
    ]]$shape_properties$override_material
  )
  expect_false(
...
test_that("Test debug channels", {
  ...
  path_debug = tempfile(
    pattern = apply(inputs, 1, paste0, collapse = "_"),
    fileext = ".png"
  )
