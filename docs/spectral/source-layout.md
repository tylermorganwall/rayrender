...
- `src/materials/spectral_material.*`: PR 13 spectral `MaterialEvalContext`, tagged Material dispatch, diffuse and interface/null material parameter objects, alpha and bump hooks, scratch-allocated `BSDF` closure construction, and a spectral material handle table; PR 16 `ConductorMaterial` with eta/k spectra, sampled reflectance compatibility, anisotropic roughness textures, remapping, and packet-preserving closure construction; PR 17 constant-ratio `DielectricMaterial` fed by `ResolvedDielectricInterface`.
...
The PR 1 render-session layer is intentionally RGB-only. PR 2 through PR 17 spectral code is
...
render-loop tests, plus constant-IOR dielectric region-state and smooth dielectric path tests.
