...
- `src/base/`: explicit RGB/XYZ/color-space types, `SampledSpectrum`, `SampledWavelengths`, tagged dispatch handles, `ScratchBuffer`, BxDF flags, and optional sample-result conventions for future pbrt-style transport.
...
The PR 1 render-session layer is intentionally RGB-only. PR 2 and PR 3 base types are compile-time
scaffolding and must remain unused by legacy transport except for isolated conversion, packet, and
allocation tests. pbrt spectral assets are introduced in later PRs.
