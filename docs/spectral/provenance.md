...
| pbrt-v4 RandomWalk integrator contracts | `src/render/spectral_integrator.h`; `src/render/spectral_integrator.cpp`; `tools/spectral-tests/pr14-randomwalk-tests.cpp` | Adapted from `src/pbrt/cpu/integrators.cpp` and pbrt `RayIntegrator`/`RandomWalkIntegrator` ordering at pinned pbrt commit `8c19f304558fd7681e2fef2c395a689d0106fb05`, with rayrender Film, Camera, Scene, Material, Light, and `ScratchBuffer` APIs preserved | Apache-2.0 | Source-derived implementation material introduced in PR 14; no scene, image, or measured rendering assets are copied | `inst/COPYRIGHTS` records pbrt notice |
