...
| pbrt-v4 BxDF/BSDF and scattering contracts | `src/render/spectral_bsdf.h`; `src/render/spectral_bsdf.cpp`; `tools/spectral-tests/pr12-bsdf-tests.cpp`; `tools/spectral-tests/pr16-conductor-tests.cpp` | Adapted from `src/pbrt/base/bxdf.h`, `src/pbrt/bxdfs.h`, `src/pbrt/bsdf.h`, and `src/pbrt/util/scattering.h` at pinned pbrt commit `8c19f304558fd7681e2fef2c395a689d0106fb05`, with the standalone diffuse/null foundation, Fresnel, Trowbridge-Reitz, BSDF frame wrapper, and PR 16 conductor reflection closure ported | Apache-2.0 | Source-derived implementation material introduced in PR 12 and extended in PR 16; no new measured optical constants or material assets are copied | `inst/COPYRIGHTS` records pbrt notice |
| pbrt-v4 Material closure contracts | `src/materials/spectral_material.h`; `src/materials/spectral_material.cpp`; `tools/spectral-tests/pr13-material-tests.cpp`; `tools/spectral-tests/pr16-conductor-tests.cpp` | Adapted from `src/pbrt/base/material.h` and `src/pbrt/materials.h` at pinned pbrt commit `8c19f304558fd7681e2fef2c395a689d0106fb05`, with rayrender texture/context and `ScratchBuffer` APIs preserved; PR 16 adds conductor eta/k and reflectance-compatibility material construction | Apache-2.0 | Source-derived implementation material introduced in PR 13 and extended in PR 16; conductor eta/k values reuse the named-spectrum assets introduced in PR 4 | `inst/COPYRIGHTS` records pbrt notice |
| pbrt-v4 RandomWalk and Path integrator contracts | `src/render/spectral_integrator.h`; `src/render/spectral_integrator.cpp`; `tools/spectral-tests/pr14-randomwalk-tests.cpp`; `tools/spectral-tests/pr15-path-tests.cpp` | Adapted from `src/pbrt/cpu/integrators.cpp` and pbrt `RayIntegrator`/`RandomWalkIntegrator`/`PathIntegrator` ordering at pinned pbrt commit `8c19f304558fd7681e2fef2c395a689d0106fb05`, with rayrender Film, Camera, Scene, Material, Light, `UniformLightSampler`, and `ScratchBuffer` APIs preserved | Apache-2.0 | Source-derived implementation material introduced in PR 14 and extended in PR 15; no scene, image, or measured rendering assets are copied | `inst/COPYRIGHTS` records pbrt notice |
