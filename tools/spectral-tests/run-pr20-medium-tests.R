...
compile_test = function(label, repo_root, extra_flags = character()) {
  ...
  build_dir = tempfile(paste0("rayrender-pr20-", label, "-"))
  dir.create(build_dir)
  executable = file.path(build_dir, paste0("pr20-medium-tests-", label))

  sources = file.path(
    repo_root,
    c(
      "tools/spectral-tests/pr20-medium-tests.cpp",
      "src/render/spectral_medium.cpp",
  ...
  message("Compiling PR20 Medium tests (", label, ")")
  run_command(cxx, args)
  executable
}

run_test = function(label, executable, extra_env = character()) {
  message("Running PR20 Medium tests (", label, ")")
...
if (!isTRUE(as.logical(Sys.getenv("RAYRENDER_PR20_SKIP_SANITIZER", "false")))) {
