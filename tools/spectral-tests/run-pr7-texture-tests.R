...
compile_test = function(label, repo_root, extra_flags = character()) {
  ...
  build_dir = tempfile(paste0("rayrender-pr7-", label, "-"))
  dir.create(build_dir)
  executable = file.path(build_dir, paste0("pr7-texture-tests-", label))

  sources = file.path(
    repo_root,
    c(
      "tools/spectral-tests/pr7-texture-tests.cpp",
  ...
  message("Compiling PR7 texture tests (", label, ")")
  run_command(cxx, args)
  executable
}

run_test = function(label, executable, env = character()) {
  message("Running PR7 texture tests (", label, ")")
  run_command(executable, character(), env = env)
}
...
if (!isTRUE(as.logical(Sys.getenv("RAYRENDER_PR7_SKIP_SANITIZER", "false")))) {
