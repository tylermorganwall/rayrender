...
compile_test = function(label, repo_root, extra_flags = character()) {
  ...
  build_dir = tempfile(paste0("rayrender-pr8-", label, "-"))
  dir.create(build_dir)
  executable = file.path(build_dir, paste0("pr8-film-tests-", label))

  sources = file.path(
    repo_root,
    c(
      "tools/spectral-tests/pr8-film-tests.cpp",
  ...
  message("Compiling PR8 Film/PixelSensor tests (", label, ")")
...
run_test = function(label, executable, asset_dir, extra_env = character()) {
  ...
  message("Running PR8 Film/PixelSensor tests (", label, ")")
...
if (!isTRUE(as.logical(Sys.getenv("RAYRENDER_PR8_SKIP_SANITIZER", "false")))) {
