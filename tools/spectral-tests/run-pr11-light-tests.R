...
compile_test = function(label, repo_root, extra_flags = character()) {
  ...
  build_dir = tempfile(paste0("rayrender-pr11-", label, "-"))
  dir.create(build_dir)
  executable = file.path(build_dir, paste0("pr11-light-tests-", label))

  sources = file.path(
    repo_root,
    c(
      "tools/spectral-tests/pr11-light-tests.cpp",
  ...
  message("Compiling PR11 Light tests (", label, ")")
  run_command(cxx, args)
  executable
}

run_test = function(label, executable, extra_env = character()) {
  message("Running PR11 Light tests (", label, ")")
...
if (!isTRUE(as.logical(Sys.getenv("RAYRENDER_PR11_SKIP_SANITIZER", "false")))) {
