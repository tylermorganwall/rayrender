...
compile_test = function(label, repo_root, extra_flags = character()) {
  ...
  build_dir = tempfile(paste0("rayrender-pr10-", label, "-"))
  dir.create(build_dir)
  executable = file.path(build_dir, paste0("pr10-scene-tests-", label))

  sources = file.path(
    repo_root,
    c(
      "tools/spectral-tests/pr10-scene-tests.cpp",
  ...
  message("Compiling PR10 Scene tests (", label, ")")
  run_command(cxx, args)
  executable
}

run_test = function(label, executable, extra_env = character()) {
  message("Running PR10 Scene tests (", label, ")")
...
if (!isTRUE(as.logical(Sys.getenv("RAYRENDER_PR10_SKIP_SANITIZER", "false")))) {
