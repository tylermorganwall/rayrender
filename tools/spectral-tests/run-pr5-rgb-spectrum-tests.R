...
compile_test = function(label, repo_root, source, extra_flags = character()) {
  ...
  build_dir = tempfile(paste0("rayrender-pr5-", label, "-"))
  dir.create(build_dir)
  executable = file.path(build_dir, paste0("pr5-rgb-spectrum-tests-", label))
  ...
  message("Compiling PR5 RGB spectrum tests (", label, ")")
...
run_test = function(label, executable, asset_dir, extra_env = character()) {
  ...
  message("Running PR5 RGB spectrum tests (", label, ")")
...
require_installed = isTRUE(as.logical(Sys.getenv(
  "RAYRENDER_PR5_REQUIRE_INSTALLED",
  "false"
)))
if (nzchar(installed_dir)) {
  ...
  stop(
    "rayrender is not installed; cannot run installed-package RGB spectrum lookup",
    call. = FALSE
  )
} else {
  message(
...
if (!isTRUE(as.logical(Sys.getenv("RAYRENDER_PR5_SKIP_SANITIZER", "false")))) {
