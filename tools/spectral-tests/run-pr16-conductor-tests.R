...
compile_test = function(label, repo_root, extra_flags = character()) {
  ...
  build_dir = tempfile(paste0("rayrender-pr16-", label, "-"))
  dir.create(build_dir)
  executable = file.path(build_dir, paste0("pr16-conductor-tests-", label))

  sources = file.path(
    repo_root,
    c(
      "tools/spectral-tests/pr16-conductor-tests.cpp",
  ...
  message("Compiling PR16 Conductor tests (", label, ")")
  run_command(cxx, args)
  executable
}

run_test = function(label, executable, extra_env = character()) {
  message("Running PR16 Conductor tests (", label, ")")
...
if (!isTRUE(as.logical(Sys.getenv("RAYRENDER_PR16_SKIP_SANITIZER", "false")))) {
