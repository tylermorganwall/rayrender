...
compile_and_run = function(
  ...
  build_dir = tempfile(paste0("rayrender-pr2-", label, "-"))
  dir.create(build_dir)
  executable = file.path(build_dir, paste0("pr2-base-tests-", label))
  ...
  message("Compiling PR2 base tests (", label, ")")
  run_command(cxx, args)
  message("Running PR2 base tests (", label, ")")
...
source = file.path(repo_root, "tools", "spectral-tests", "pr2-base-tests.cpp")

compile_and_run("normal", repo_root, source)

if (!isTRUE(as.logical(Sys.getenv("RAYRENDER_PR2_SKIP_SANITIZER", "false")))) {
