...
compile_test = function(label, repo_root, extra_flags = character()) {
  ...
  build_dir = tempfile(paste0("rayrender-pr12-", label, "-"))
  dir.create(build_dir)
  executable = file.path(build_dir, paste0("pr12-bsdf-tests-", label))

  sources = file.path(
    repo_root,
    c(
      "tools/spectral-tests/pr12-bsdf-tests.cpp",
  ...
  message("Compiling PR12 BSDF tests (", label, ")")
  run_command(cxx, args)
  executable
}

run_test = function(label, executable, extra_env = character()) {
  message("Running PR12 BSDF tests (", label, ")")
...
if (!isTRUE(as.logical(Sys.getenv("RAYRENDER_PR12_SKIP_SANITIZER", "false")))) {
