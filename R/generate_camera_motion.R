...
#' `exp` interpolate camera orientation with shortest-path quaternion slerp.
...
generate_camera_motion = function(
  ...
  curve_adj_type = unlist(lapply(
    tolower(curvature_adjust),
    switch,
    "none" = 0,
    "position" = 1,
    "lookats" = 2,
  ...
  if (type == "bezier") {
    ...
    points_dist = calculate_distance_along_bezier_curve(
      position_control_points,
      breaks = 50
    )
    points_final = calculate_final_path(
      points_dist,
      steps = frames,
    ...
    if (!is.null(lookats)) {
      position_control_lookats = process_point_series(
        lookats,
        closed = closed,
        straight = FALSE
      )
      lookats_dist = calculate_distance_along_bezier_curve(
        position_control_lookats,
        breaks = 50
      )
      lookats_final = calculate_final_path(
        lookats_dist,
        steps = frames,
        constant_step = constant_step,
        offset = offset_lookat,
      ...
      lookats_final = matrix(
        c(0, 0, 0),
        nrow = nrow(points_final),
        ncol = 3,
        byrow = TRUE
      )
    }
    ...
    if (length(apertures) != 1) {
      apertures_control_lookats = process_point_series_1d(
        apertures,
        closed = closed,
        straight = aperture_linear
      )
      apertures_dist = calculate_distance_along_bezier_curve(
        apertures_control_lookats,
        breaks = 50
      )
      apertures_final = calculate_final_path(
        apertures_dist,
        steps = frames,
        constant_step = constant_step
      )
    ...
    if (length(fovs) != 1) {
      fovs_control_lookats = process_point_series_1d(
        fovs,
        closed = closed,
        straight = fov_linear
      )
      fovs_dist = calculate_distance_along_bezier_curve(
        fovs_control_lookats,
        breaks = 50
      )
      fovs_final = calculate_final_path(
        fovs_dist,
        steps = frames,
        constant_step = constant_step
      )
    ...
    if (!is.null(focal_distances)) {
      if (is.numeric(focal_distances)) {
        ...
        focal_control_lookats = process_point_series_1d(
          focal_distances,
          closed = closed,
          straight = focal_linear
        )
        focal_dist = calculate_distance_along_bezier_curve(
          focal_control_lookats,
          breaks = 50
        )
        focal_final = calculate_final_path(
          focal_dist,
          steps = frames,
          constant_step = constant_step
        )
    ...
    if (!is.null(ortho_dims)) {
      ortho_control_lookats = process_point_series_2d(
        ortho_dims,
        closed = closed,
        straight = focal_linear
      )
      ortho_dist = calculate_distance_along_bezier_curve(
        ortho_control_lookats,
        breaks = 50
      )
      ortho_final = calculate_final_path(
        ortho_dist,
        steps = frames,
        constant_step = constant_step
      )
    ...
    if (!is.null(camera_ups)) {
      ups_control = process_point_series_2d(
        camera_ups,
        closed = closed,
        straight = ortho_linear
      )
      up_dist = calculate_distance_along_bezier_curve(ups_control, breaks = 50)
      up_final = calculate_final_path(
        up_dist,
        steps = frames,
        constant_step = constant_step
      )
    ...
    final_motion = as.data.frame(cbind(
      points_final,
      lookats_final,
      apertures_final,
      fovs_final,
    ...
    if (is.null(focal_distances)) {
      focal_distances = sqrt(
        (positions$x - lookats$x)^2 +
    ...
    final_motion = slerp_camera_motion_orientation(final_motion, tween_df, type)
    return(as_ray_camera_motion(final_motion))
  } else if (type == "manual") {
    if (inherits(positions, "list")) {
      positions = do.call(rbind, positions)
...
#' Slerp Camera Motion Orientation
#'
#' @param motion Motion data frame.
#' @param keyframes Keyframe data frame.
#' @param ease Interpolation type.
#' @return Motion data frame with slerped orientation columns.
#'
#' @keywords internal
slerp_camera_motion_orientation = function(motion, keyframes, ease = "linear") {
  if (nrow(motion) == 0 || nrow(keyframes) < 2) {
    return(motion)
  }
  orientation_progress = camera_orientation_progress(
    keyframe_count = nrow(keyframes),
    frames = nrow(motion),
    ease = ease
  )
  keyframe_quaternions = camera_keyframe_quaternions(keyframes)
  look_distances = camera_keyframe_look_distances(keyframes)
  look_distances = tween(look_distances, n = nrow(motion), ease = ease)
  positions = as.matrix(motion[, c("x", "y", "z")])
  lookats = matrix(0, nrow = nrow(motion), ncol = 3)
  ups = matrix(0, nrow = nrow(motion), ncol = 3)

  for (i in seq_len(nrow(motion))) {
    segment = orientation_progress$segment[i]
    t = orientation_progress$t[i]
    orientation = quaternion_slerp(
      keyframe_quaternions[segment, ],
      keyframe_quaternions[segment + 1, ],
      t
    )
    basis = quaternion_to_camera_basis(orientation)
    forward = normalize_camera_vector(basis[, 3], c(0, 0, 1))
    up = normalize_camera_vector(basis[, 2], c(0, 1, 0))
    lookats[i, ] = positions[i, ] + forward * look_distances[i]
    ups[i, ] = up
  }

  motion[, c("dx", "dy", "dz")] = lookats
  motion[, c("upx", "upy", "upz")] = ups
  return(motion)
}

#' Camera Orientation Progress
#'
#' @param keyframe_count Number of keyframes.
#' @param frames Number of output frames.
#' @param ease Interpolation type.
#' @return Data frame of orientation segment indices and interpolation values.
#'
#' @keywords internal
camera_orientation_progress = function(
  keyframe_count,
  frames,
  ease = "linear"
) {
  if (frames < keyframe_count) {
    stop(
  ...
  if (frames == 1 || keyframe_count == 1) {
    return(data.frame(segment = 1L, t = 0))
  }

  segment_count = keyframe_count - 1
  len_vals = rep(0, segment_count)
  free_vals = frames - keyframe_count
  counter = 1
  for (i in seq_len(free_vals)) {
    len_vals[counter] = len_vals[counter] + 1
    counter = counter + 1
    if (counter > segment_count) {
      counter = 1
    }
  }

  progress = vector("list", frames)
  counter = 1
  for (segment in seq_len(segment_count)) {
    t = seq(0, 1, length.out = len_vals[segment] + 2)[
      -(len_vals[segment] + 2)
    ]
    if (ease == "cubic") {
      t = cubicInOut(t)
    ...
    for (tval in t) {
      progress[[counter]] = data.frame(segment = segment, t = tval)
      counter = counter + 1
    }
  }
  progress[[counter]] = data.frame(segment = segment_count, t = 1)
  return(do.call(rbind, progress))
}

#' Camera Keyframe Quaternions
#'
#' @param keyframes Keyframe data frame.
#' @return Matrix of camera orientation quaternions.
#'
#' @keywords internal
camera_keyframe_quaternions = function(keyframes) {
  positions = as.matrix(keyframes[, c("x", "y", "z")])
  lookats = as.matrix(keyframes[, c("dx", "dy", "dz")])
  ups = as.matrix(keyframes[, c("upx", "upy", "upz")])
  quaternions = matrix(0, nrow = nrow(keyframes), ncol = 4)

  for (i in seq_len(nrow(keyframes))) {
    basis = camera_basis_from_lookat(positions[i, ], lookats[i, ], ups[i, ])
    quaternions[i, ] = camera_basis_to_quaternion(basis)
  }
  return(quaternions)
}

#' Camera Keyframe Look Distances
#'
#' @param keyframes Keyframe data frame.
#' @return Numeric vector of lookat distances.
#'
#' @keywords internal
camera_keyframe_look_distances = function(keyframes) {
  positions = as.matrix(keyframes[, c("x", "y", "z")])
  lookats = as.matrix(keyframes[, c("dx", "dy", "dz")])
  look_distances = sqrt(rowSums((lookats - positions)^2))
  invalid = !is.finite(look_distances) |
    look_distances <= sqrt(.Machine$double.eps)
  if (any(invalid) && "focal" %in% colnames(keyframes)) {
    fallback = keyframes$focal
    fallback[!is.finite(fallback) | fallback <= sqrt(.Machine$double.eps)] = 1
    look_distances[invalid] = fallback[invalid]
  }
  look_distances[
    !is.finite(look_distances) | look_distances <= sqrt(.Machine$double.eps)
  ] = 1
  return(look_distances)
}

#' Camera Basis From Lookat
#'
#' @param position Camera position.
#' @param lookat Camera lookat.
#' @param up Camera up hint.
#' @return Camera basis matrix.
#'
#' @keywords internal
camera_basis_from_lookat = function(position, lookat, up) {
  forward = normalize_camera_vector(lookat - position, c(0, 0, 1))
  up = normalize_camera_vector(up, camera_default_up(forward))
  right = cross_prod(up, forward)
  if (sqrt(sum(right^2)) <= sqrt(.Machine$double.eps)) {
    up = camera_default_up(forward)
    right = cross_prod(up, forward)
  }
  right = normalize_camera_vector(right, c(1, 0, 0))
  up = normalize_camera_vector(cross_prod(forward, right), up)
  return(cbind(right, up, forward))
}
...
#'
#' @keywords internal
camera_default_up = function(forward) {
...
#'
#' @keywords internal
normalize_camera_vector = function(value, fallback) {
  value = as.numeric(value)
  length_value = sqrt(sum(value^2))
  if (!is.finite(length_value) || length_value <= sqrt(.Machine$double.eps)) {
    value = as.numeric(fallback)
    length_value = sqrt(sum(value^2))
  }
  if (!is.finite(length_value) || length_value <= sqrt(.Machine$double.eps)) {
    return(c(0, 0, 1))
...
#' Camera Basis To Quaternion
#'
#' @param basis Camera basis matrix.
#' @return Quaternion as w, x, y, z.
#'
#' @keywords internal
camera_basis_to_quaternion = function(basis) {
  trace_basis = sum(diag(basis))
  if (trace_basis > 0) {
    scale = sqrt(trace_basis + 1) * 2
    quaternion = c(
      0.25 * scale,
  ...
  } else if (basis[1, 1] > basis[2, 2] && basis[1, 1] > basis[3, 3]) {
    scale = sqrt(1 + basis[1, 1] - basis[2, 2] - basis[3, 3]) * 2
    quaternion = c(
      (basis[3, 2] - basis[2, 3]) / scale,
      0.25 * scale,
      (basis[1, 2] + basis[2, 1]) / scale,
      (basis[1, 3] + basis[3, 1]) / scale
    )
  } else if (basis[2, 2] > basis[3, 3]) {
    scale = sqrt(1 + basis[2, 2] - basis[1, 1] - basis[3, 3]) * 2
    quaternion = c(
      (basis[1, 3] - basis[3, 1]) / scale,
      (basis[1, 2] + basis[2, 1]) / scale,
      0.25 * scale,
      (basis[2, 3] + basis[3, 2]) / scale
    )
  } else {
    scale = sqrt(1 + basis[3, 3] - basis[1, 1] - basis[2, 2]) * 2
    quaternion = c(
      (basis[2, 1] - basis[1, 2]) / scale,
      (basis[1, 3] + basis[3, 1]) / scale,
      (basis[2, 3] + basis[3, 2]) / scale,
...
#' Quaternion To Camera Basis
#'
#' @param quaternion Quaternion as w, x, y, z.
#' @return Camera basis matrix.
#'
#' @keywords internal
quaternion_to_camera_basis = function(quaternion) {
...
#'
#' @keywords internal
quaternion_slerp = function(quaternion1, quaternion2, t) {
  quaternion1 = normalize_quaternion(quaternion1)
  quaternion2 = normalize_quaternion(quaternion2)
  cos_theta = sum(quaternion1 * quaternion2)
  if (cos_theta < 0) {
  ...
  if (cos_theta > 0.9995) {
    return(normalize_quaternion(
      quaternion1 * (1 - t) + quaternion2 * t
    ))
  }
  theta = acos(cos_theta)
  sin_theta = sin(theta)
  weight1 = sin((1 - t) * theta) / sin_theta
  weight2 = sin(t * theta) / sin_theta
  return(normalize_quaternion(quaternion1 * weight1 + quaternion2 * weight2))
}
...
#'
#' @keywords internal
normalize_quaternion = function(quaternion) {
  quaternion = as.numeric(quaternion)
  quaternion_length = sqrt(sum(quaternion^2))
  if (
    !is.finite(quaternion_length) ||
      quaternion_length <= sqrt(.Machine$double.eps)
  ) {
    return(c(1, 0, 0, 0))
  }
  return(quaternion / quaternion_length)
}
...
calculate_distance_along_bezier_curve = function(cps, breaks = 20) {
  ...
  if (any(temp_deriv != 0)) {
    curve = sqrt(sum(cross_prod(temp_deriv, temp_second_deriv)^2)) /
      sum(temp_deriv^2)^(3 / 2)
  } else {
    curve = Inf
  }

  distance_list[[1]] = data.frame(
    dist = 0,
    segment = 1,
    t = 0,
    x = prev_pos[1],
  ...
  for (seg in seq_len(length(cps))) {
    for (t in seq(0, 1, length.out = breaks + 1)[-1]) {
      ...
      if (any(temp_deriv != 0)) {
        curve = sqrt(sum(cross_prod(temp_deriv, temp_second_deriv)^2)) /
          sum(temp_deriv^2)^(3 / 2)
        distance_list[[counter]] = data.frame(
          dist = sqrt(sum((temp_pos - prev_pos)^2)),
          segment = seg,
          t = (seg + t - 1) / length(cps),
          x = temp_pos[1],
          y = temp_pos[2],
          z = temp_pos[3],
          curvature = curve,
  ...
  if (constant_step) {
    ...
    if (!curvature_adjust) {
      ...
      if (offset != 0) {
        ...
        direction = direction / sqrt(sum(direction^2))
        final_points[[length(rowvals)]] = linearized_cp[
          nrow(linearized_cp),
          c("x", "y", "z")
        ] +
          direction * offset
      ...
      if (progress) {
        pb = progress::progress_bar$new(
          format = sprintf("Generating Camera %s [:bar] :percent", string),
          total = maxdist,
          width = 70
        )
      }
...
get_saved_keyframes = function() {
  ...
  if (nrow(keyframes) == 0) {
    message(
      "No keyframes saved: press K when using interactive preview to save keyframe"
    )
    return(keyframes)
  }
