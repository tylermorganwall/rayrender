...
print.ray_animated_transform = function(x, ...) {
  ...
  fmt = function(x) {
    cli::ansi_columns(
      text = paste0(
        paste(cli::col_red(names(x)), cli::col_grey(x), sep = ": "),
        collapse = " "
      ),
      width = 100,
      fill = "rows",
...
format_pillar_animated_transform = function(x) {
  format_material <- function(x) {
    ...
    if (animated) {
      ...
      output_desc[5] = pillar::style_subtle(cli::col_red(sprintf(
        "%0.1f",
        x$end_time
      )))
    }
...
pillar_shaft.ray_animated_transform = function(x, ...) {
  pillar::new_pillar_shaft_simple(
    format_pillar_animated_transform(x),
    width = 10
  )
}
