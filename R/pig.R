...
pig = function(
  ...
  if (spider) {
    legs_mat = list()
    spider_angles = c(
      seq(-40, 40, length.out = 4),
      -seq(140, 220, length.out = 4)
    )
    for (i in 1:8) {
      ...
      legs_mat[[i]] = matrix(
        c(c(0, 0, 0), vec_ret(2, bangle, 2), vec_ret(3, bangle, -1)),
        ncol = 3,
        byrow = TRUE
      )
    }
  ...
  pig = group_objects(
    ellipsoid(y = 1, a = 2, b = 1, c = 1, material = bodymat) %>%
      ...
      add_object(segment(
        start = c(2, 2.8 + eyebrow_offset_left[2], -0.5),
        end = c(2, 2.8 + eyebrow_offset_left[1], -0.1),
        radius = 0.05,
        material = diffuse(color = "black")
      )) %>%
      add_object(segment(
        start = c(2, 2.8 + eyebrow_offset_right[1], 0.5),
        end = c(2, 2.8 + eyebrow_offset_right[2], 0.1),
        radius = 0.05,
        material = diffuse(color = "black")
      )),
    scale = scale,
