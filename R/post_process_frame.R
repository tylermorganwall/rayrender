...
post_process_frame = function(
  ...
  full_array[,, 1] = fliplr(flipud(t(rgb_mat$r)))
  full_array[,, 2] = fliplr(flipud(t(rgb_mat$g)))
  full_array[,, 3] = fliplr(flipud(t(rgb_mat$b)))
  if (transparent_background) {
  ...
  if (debug_channel == 1) {
    ...
    output_array = (full_array - min(full_array, na.rm = TRUE)) /
      (max(full_array, na.rm = TRUE) - min(full_array, na.rm = TRUE))
    if (write_file) {
      rayimage::ray_write_image(output_array, filename)
    } else if (plot_scene) {
