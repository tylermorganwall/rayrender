...
vec3f triangle_image_texture::value(Float u, Float v, const vec3f& p) const {
...
vec3f alpha_texture::value(Float u, Float v, const vec3f& p) const {
  int i = u * nx;
  int j = (1-v) * ny;
  if (i < 0) i = 0;
  if (j < 0) j = 0;
...
Float alpha_texture::channel_value(Float u, Float v, const vec3f& p) const {
...
vec3f bump_texture::value(Float u, Float v, const vec3f& p) const {
  while(u < 0) u += 1;
  while(v < 0) v += 1;
  while(u > 1) u -= 1;
  while(v > 1) v -= 1;
  ...
  return(vec3f(intensity*bu,intensity*bv,0));
}

vec3f bump_texture::mesh_value(Float u, Float v, const vec3f& p) const {
  Float uu = ((1 - u - v) * u_vec.x() + u * u_vec.y() + v * u_vec.z());
  Float vv = ((1 - u - v) * v_vec.x() + u * v_vec.y() + v * v_vec.z());
  while(uu < 0) uu += 1;
  while(vv < 0) vv += 1;
