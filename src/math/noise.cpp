...
Float Noise(Float x, Float y, Float z) {
    ...
    // int32s.
    x = std::fmod(x, Float(1 << 30));
    y = std::fmod(y, Float(1 << 30));
    z = std::fmod(z, Float(1 << 30));
...
vec3f DNoise(point3f p) {
    Float delta = .01f;
    Float n = Noise(p);
    point3f noiseDelta(Noise(p + vec3f(delta, 0, 0)), Noise(p + vec3f(0, delta, 0)),
                       Noise(p + vec3f(0, 0, delta)));
    return (noiseDelta - point3f(n, n, n)) / delta;
}

Float FBm(point3f p, vec3f dpdx, vec3f dpdy, Float omega, int maxOctaves) {
    // Compute number of octaves for antialiased FBm
    Float len2 = std::max((dpdx).squared_length(), (dpdy).squared_length());
...
Float Turbulence(point3f p, vec3f dpdx, vec3f dpdy, Float omega, int maxOctaves) {
    // Compute number of octaves for antialiased FBm
    Float len2 = std::max((dpdx).squared_length(), (dpdy).squared_length());
    Float n = clamp(-1 - Log2(len2) / 2, 0, maxOctaves);
    int nInt = std::floor(n);
    ...
    Float nPartial = n - nInt;
    sum += o * lerp(SmoothStep(nPartial, .3f, .7f), (Float)0.2, std::abs(Noise(lambda * p)));
