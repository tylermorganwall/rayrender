...
public:
  ...
  point3f emitted(const Ray &ray, const hit_record &, Float, Float, const point3f &,
    ...
    invisible = false;
    return light->Radiance(ray.origin(), ray.direction(), ray.time());
  }
...
InfiniteAreaLight::InfiniteAreaLight(std::shared_ptr<InfiniteLight> source, Float r,
    ...
    : hitable(to_world, to_light, std::make_shared<InfiniteLightMaterial>(source), false),
...
const bool InfiniteAreaLight::hit(const Ray& r, Float t_min, Float t_max, hit_record& rec, random_gen& rng) const {
  ...
  if(temp1 < t_max && temp1 > t_min) {
    ...
    get_sphere_uv_z(unit_vector(r2.direction()), rec.u, rec.v);
    // rec.u = 1 - rec.u;
...
const bool InfiniteAreaLight::hit(const Ray& r, Float t_min, Float t_max, hit_record& rec, Sampler* sampler) const {
  ...
  //   rec.normal = convert_to_normal3(-(r2.direction()));
    ...
    // vec3f v(r2.direction().xyz.z,r2.direction().xyz.y,r2.direction().xyz.x);
    get_sphere_uv_z(unit_vector(r2.direction()), rec.u, rec.v);
    // rec.u = 1 - rec.u;
  ...
  //   rec.normal = convert_to_normal3(-(r2.direction()));
    ...
    //   //Interaction information
    //   Float zRadius = std::sqrt(rec.p.xyz.x * rec.p.xyz.x  + rec.p.xyz.z  *
    //   rec.p.xyz.z ); Float invZRadius = 1 / zRadius; Float cosPhi =
    //   rec.p.xyz.x * invZRadius; Float sinPhi = rec.p.xyz.z * invZRadius;
    //   Float theta = std::acos(clamp(rec.p.xyz.z / radius, -1, 1));
    //   rec.dpdu = 2 * static_cast<Float>(M_PI) * vec3f(-rec.p.xyz.z, 0,
    //   rec.p.xyz.x); rec.dpdv = 2 * static_cast<Float>(M_PI) *
...
Float InfiniteAreaLight::pdf_value(const point3f& o, const vec3f& v, random_gen& rng, Float time) {
  if (light) return light->Pdf(o, v, time);
  hit_record rec;
  // if(this->hit(ray(o,v), 0.001, FLT_MAX, rec, rng)) {
    vec3f d = (*WorldToObject)(v);
    // vec3f v2(d.xyz.z,d.xyz.y,d.xyz.x);
    get_sphere_uv_z(unit_vector(d), rec.u, rec.v);
    // rec.u = 1 - rec.u;
...
vec3f InfiniteAreaLight::random(const point3f& o, random_gen& rng, Float time) {
  ...
  if (light) return light->Sample(o, u, time);
  Float mapPdf;
  ...
  Float theta = (1 - uv[1]) * static_cast<Float>(M_PI),
        phi = (-uv[0]) * 2.0f * static_cast<Float>(M_PI);
...
vec3f InfiniteAreaLight::random(const point3f& o, Sampler* sampler, Float time) {
  ...
  //theta vertical, phi horizontal
  Float theta = (1 - uv[1]) * static_cast<Float>(M_PI), phi = (-uv[0]) * 2.0f * static_cast<Float>(M_PI);
  Float cosTheta = std::cos(theta), sinTheta = std::sin(theta);
  Float sinPhi = std::sin(phi), cosPhi = std::cos(phi);
  vec3f d(sinTheta * sinPhi, cosTheta, sinTheta * cosPhi);
  return((*ObjectToWorld)(d));
