...
class csg_torus : public ImplicitShape { 
  public: 
    ...
    Float getDistance(const point3f& from_old) const {
      vec3f from = from_old - center;
      vec2f q = vec2f(std::sqrt(from.xyz.x*from.xyz.x + from.xyz.z*from.xyz.z) - ring_radius, from.xyz.y);
...
class csg_rounded_cone : public ImplicitShape { 
  public: 
    ...
    virtual bool bbox(Float t0, Float t1, aabb& box) const {
      point3f min = point3f(ffmin(start.xyz.x,end.xyz.x),ffmin(start.xyz.y,end.xyz.y),ffmin(start.xyz.z,end.xyz.z));
      point3f max = point3f(ffmax(start.xyz.x,end.xyz.x),ffmax(start.xyz.y,end.xyz.y),ffmax(start.xyz.z,end.xyz.z));
...
class csg_cone : public ImplicitShape { 
  public: 
    ...
    Float getDistance(const point3f& from_old) const {
      ...
      vec2f w = vec2f( std::sqrt(from.xyz.x * from.xyz.x +  from.xyz.z *  from.xyz.z), from.xyz.y );
      vec2f a = w - q*clamp( dot(w,q)/dot(q,q), 0.0, 1.0 );
      vec2f b = w - q*vec2f( clamp( w.xy.x/q.xy.x, 0.0, 1.0 ), 1.0 );
      float k = sgn( q.xy.y );
      float d = std::min(dot( a, a ),dot(b, b));
      float s = std::fmax( k*(w.xy.x*q.xy.y-w.xy.y*q.xy.x),k*(w.xy.y-q.xy.y));
...
class csg_pyramid : public ImplicitShape { 
  public: 
    ...
    Float getDistance(const point3f& from_old) const {
      ...
      from = (from.xyz.z > from.xyz.x) ? vec3f(from.xyz.z,from.xyz.y,from.xyz.x) : from;
      from -= vec3f(0.5,0,0.5);
      
      vec3f q = vec3f(from.xyz.z, h*from.xyz.y - 0.5*from.xyz.x, h*from.xyz.x + 0.5*from.xyz.y);
      
      float s = std::fmax(-q.xyz.x,0.0);
      float t = clamp((q.xyz.y-0.5*from.xyz.z)*m2_inv_buff, 0.0, 1.0 );
      
      float a = m2*(q.xyz.x+s)*(q.xyz.x+s) + q.xyz.y*q.xyz.y;
      float b = m2*(q.xyz.x+0.5*t)*(q.xyz.x+0.5*t) + (q.xyz.y-m2*t)*(q.xyz.y-m2*t);
      
      float d2 = std::fmin(q.xyz.y,-q.xyz.x*m2-q.xyz.y*0.5) > 0.0 ? 0.0 : std::fmin(a,b);
      
      return(std::sqrt((d2+q.xyz.z*q.xyz.z) * m2_inv ) * sgn(std::fmax(q.xyz.z,-from.xyz.y)));
    } 
    virtual bool bbox(Float t0, Float t1, aabb& box) const {
...
class csg_triangle : public ImplicitShape { 
  public: 
    ...
    virtual bool bbox(Float t0, Float t1, aabb& box) const {
      point3f min = point3f(ffmin(ffmin(a.xyz.x,b.xyz.x),c.xyz.x),
                      ffmin(ffmin(a.xyz.y,b.xyz.y),c.xyz.y),
                      ffmin(ffmin(a.xyz.z,b.xyz.z),c.xyz.z));
      
      point3f max = point3f(ffmax(ffmax(a.xyz.x,b.xyz.x),c.xyz.x),
...
class csg_rotate : public ImplicitShape { 
  public: 
    csg_rotate(std::shared_ptr<ImplicitShape> shape, point3f pivot_point_, vec3f up) : 
      ...
      corners[2] = axis.local_to_world(point3f(box2.min().xyz.x, box2.max().xyz.y, box2.min().xyz.z)- pivot_point) + pivot_point;
      corners[3] = axis.local_to_world(point3f(box2.max().xyz.x, box2.min().xyz.y, box2.min().xyz.z)- pivot_point) + pivot_point;
      corners[4] = axis.local_to_world(point3f(box2.min().xyz.x, box2.max().xyz.y, box2.max().xyz.z)- pivot_point) + pivot_point;
      corners[5] = axis.local_to_world(point3f(box2.max().xyz.x, box2.min().xyz.y, box2.max().xyz.z)- pivot_point) + pivot_point;
      ...
      for(int i = 1; i < 7; i++) {
        ...
        temp_max = point3f(ffmax(temp_max.xyz.x, corners[i].xyz.x),
                        ffmax(temp_max.xyz.y, corners[i].xyz.y),
                        ffmax(temp_max.xyz.z, corners[i].xyz.z));
      }
