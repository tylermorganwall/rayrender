...
public:
  ...
  void SetToneMap(const std::string& name) {
    if(name == "raw") method = ToneMap::Raw;
    else if(name == "hbd") method = ToneMap::Hbd;
    else if(name == "reinhard") method = ToneMap::Reinhard;
    else if(name == "uncharted") method = ToneMap::Uncharted;
    else throw std::invalid_argument("Unknown preview tone map: " + name);
  }
  ...
  static double Luminance(const RGB& rgb) {
    ...
    return 0.2126390058715103 * rgb[0] +
           0.7151686787677559 * rgb[1] + 0.07219231536073371 * rgb[2];
  }

  static double EncodeSRGB(double linear) {
    if(std::isnan(linear)) return 0;
  ...
  RGB Apply(RGB rgb) const {
    if(NeedsLogAverage()) {
      ...
      for(double& value : rgb) value *= scale;
    }
