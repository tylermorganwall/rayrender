  ...
  for (const auto& root : roots) {
    if (root.id == id) {
      ...
      for (size_t slot = 0; slot < materials.size(); ++slot) {
        ...
        for (const auto& binding : PreviewMaterialFields(materials[slot])) {
          panel.fields.push_back(binding.field);
        }
        if (materials[slot] == picked) {
          ui.material_slot = static_cast<int32_t>(ui.materials.size());
        }
...
void PreviewScene::Describe(uint64_t id, PreviewObjectState& ui, material* picked) {
  ...
  ui.label = group
                 ? "Group " + std::to_string(id & ~(UINT64_C(1) << 63)) + " (" +
                       std::to_string(count) + " objects)"
                 : first->object->GetName() + " " + std::to_string(first->key.row + 1) +
                       " / " + std::to_string(first->key.placement + 1);
...
// Intersect selectable roots and keep the nearest accepted hit. Intersections
// may descend into meshes/instances, but selection always reports the outer ID.
bool PreviewScene::Pick(const Ray& ray, PreviewObjectState& ui,
                        const std::function<bool()>& cancel) {
  Float closest = std::numeric_limits<Float>::infinity();
  uint64_t id = 0;
  material* picked = nullptr;
  // Picking has its own deterministic random stream and cannot advance rendering RNGs.
  random_gen rng(1);
  for (size_t i = 0; i < roots.size(); ++i) {
    ...
    hit_record rec;
    if (roots[i].object->hit(ray, .001f, closest, rec, rng) && !rec.alpha_miss &&
        std::isfinite(rec.t)) {
      closest = rec.t;
      id = roots[i].id;
      picked = rec.mat_ptr;
...
bool PreviewScene::Apply(PreviewObjectState& ui) {
  ...
  try {
    ...
    if (transform) {
      ...
      for (const auto& root : roots) {
        if (root.id == ui.id) {
          next.settings.transforms[root.key] =
              delta * ObjectTransform(root.key.row, root.key.placement);
        }
      }
    }
    if (material_edit) {
      for (const auto& panel : ui.materials) {
        if (panel.changed) {
          next.settings.materials[{panel.row, panel.placement}][panel.slot] = {
              panel.type, panel.fields};
        }
      }
    }
    if (revert) {
      for (const auto& root : roots) {
        if (root.id == ui.id) {
          next.settings.transforms.erase(root.key);
          next.settings.materials.erase(root.key);
        }
      }
    }
    if (!prepare_rebuild) {
...
// material-slot indices become one-based for R; transforms remain world-space
// deltas relative to the input scene, not absolute gizmo placement matrices.
Rcpp::List PreviewScene::ExportEdits() const {
  ...
  for (const auto& root : roots) {
    if (settings.transforms.count(root.key) || settings.materials.count(root.key)) {
      ...
      result.push_back(Rcpp::List::create(Rcpp::_["row"] = root.key.row + 1,
