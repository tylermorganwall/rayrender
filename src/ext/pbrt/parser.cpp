...
// TMW: My additions below to remove dep on double_conversion lib

#include <cstdlib>
#include <string>

inline double parseDoubleNew(const char* buf, std::size_t len, int* consumed) {
  // Make a null-terminated copy.
  std::string tmp(buf, len);
  char* end = nullptr;
  double val = std::strtod(tmp.c_str(), &end);
  *(consumed) = static_cast<int>(end - tmp.c_str());
  return val;
}

inline float parseFloatNew(const char* buf, std::size_t len, int* consumed) {
  // Make a null-terminated copy.
  std::string tmp(buf, len);
  char* end = nullptr;
...
static double parseFloat(const Token &t) {
    ...
    if (isInteger(t.token)) {
        ...
        val = parseFloatNew(bufp, t.token.size(), &length); // TMW: Changed to remove ext dep
    else
        val = parseDoubleNew(bufp, t.token.size(), &length); // TMW: Changed to remove ext dep
